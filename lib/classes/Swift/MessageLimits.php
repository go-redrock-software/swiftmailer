...
class Swift_MessageLimits
{
    ...
    public function validate(Swift_Mime_SimpleMessage $message): void
    {
        ...
        if ($recipientCount > $this->maxRecipientCount) {
            throw new Swift_SwiftException(\sprintf('Message has %d recipients, maximum is %d', $recipientCount, $this->maxRecipientCount));
        }

        $bodySize = \strlen($message->getBody() ?? '');
        if ($bodySize > $this->maxBodySize) {
            throw new Swift_SwiftException(\sprintf('Message body is %d bytes, maximum is %d', $bodySize, $this->maxBodySize));
        }
        ...
        foreach ($message->getChildren() ?? [] as $child) {
            if ($child instanceof Swift_Attachment || $child instanceof Swift_Image) {
                ...
                if ($attachSize > $this->maxAttachmentSize) {
                    throw new Swift_SwiftException(\sprintf('Attachment "%s" is %d bytes, maximum is %d', $child->getFilename(), $attachSize, $this->maxAttachmentSize));
                }
            }
        }
        if ($attachmentCount > $this->maxAttachmentCount) {
            throw new Swift_SwiftException(\sprintf('Message has %d attachments, maximum is %d', $attachmentCount, $this->maxAttachmentCount));
        }
        if ($totalSize > $this->maxTotalSize) {
            throw new Swift_SwiftException(\sprintf('Total message size is %d bytes, maximum is %d', $totalSize, $this->maxTotalSize));
        }
    }
}
