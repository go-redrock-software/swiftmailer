...
class Swift_Transport_RetryTransportTest extends PHPUnit\Framework\TestCase
{
    ...
    public function testRetryRestartsStoppedTransportEvenWhenStartThrows()
    {
        $inner   = $this->createMock(Swift_Transport::class);
        $message = (new Swift_Message())
            ->setFrom(['sender@example.com'])
        ...
        // Transport reports not started after first failure
        $inner->method('isStarted')
            ->willReturnOnConsecutiveCalls(false, false, true);

        // First start() throws, second start() succeeds
        $startCallCount = 0;
        $inner->method('start')
            ->willReturnCallback(function () use (&$startCallCount) {
                ++$startCallCount;
                if (1 === $startCallCount) {
                    throw new Swift_TransportException('Start failed');
                }
            });
    ...
    public function testBackoffWithPositiveDelayExecutesUsleep()
    {
        $inner   = $this->createMock(Swift_Transport::class);
        $message = (new Swift_Message())
            ->setFrom(['sender@example.com'])
        ...
        // Use a very small delay to keep the test fast
        $retry  = new Swift_Transport_RetryTransport($inner, maxRetries: 1, baseDelayMs: 1);
        $result = $retry->send($message);
        $this->assertSame(1, $result);
    }
}
